...
BEGIN
    ...
    BEGIN
        RAISERROR('QuestionText cannot be empty.', 16, 1);
        RETURN;
    END

    IF @QuestionType NOT IN ('MCQ', 'TF')
    BEGIN
        RAISERROR('QuestionType must be MCQ or TF.', 16, 1);
        RETURN;
    END
...
BEGIN
    ...
    BEGIN
        RAISERROR('InstructorName cannot be empty.', 16, 1);
        RETURN;
    END

    IF @Email IS NOT NULL AND EXISTS (SELECT 1 FROM Instructor WHERE Email = @Email)
    BEGIN
...
BEGIN
    ...
    IF @InstructorName IS NOT NULL AND LTRIM(RTRIM(@InstructorName)) = ''
    BEGIN
        RAISERROR('InstructorName cannot be empty.', 16, 1);
        RETURN;
    END

    IF @Email IS NOT NULL AND EXISTS (SELECT 1 FROM Instructor WHERE Email = @Email AND InstructorID <> @InstructorID)
    BEGIN
        RAISERROR('Email already exists.', 16, 1);
        RETURN;
    END
    ...
    BEGIN TRY
        ...
        UPDATE Instructor
        SET InstructorName = COALESCE(@InstructorName, InstructorName),
            Email = COALESCE(@Email, Email),
            DepartmentNo = COALESCE(@DepartmentNo, DepartmentNo)
...
BEGIN
    ...
    SELECT InstructorID, InstructorName, Email, DepartmentNo
    FROM Instructor
    WHERE (@InstructorID IS NULL OR InstructorID = @InstructorID)
      AND (@DepartmentNo IS NULL OR DepartmentNo = @DepartmentNo);
END;
GO
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM Instructor_Course WHERE InstructorID = @InstructorID AND CourseID = @CourseID)
    BEGIN
...
BEGIN
    ...
    IF @InstructorID IS NULL OR @CourseID IS NULL OR @NewInstructorID IS NULL OR @NewCourseID IS NULL
    BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM Instructor WHERE InstructorID = @NewInstructorID)
    BEGIN
        RAISERROR('New Instructor not found.', 16, 1);
        RETURN;
    END
    ...
    IF EXISTS (SELECT 1 FROM Instructor_Course WHERE InstructorID = @NewInstructorID AND CourseID = @NewCourseID)
    BEGIN
